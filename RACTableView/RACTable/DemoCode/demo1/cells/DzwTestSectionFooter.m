...
@interface DzwTestSectionFooter ()
@property (nonatomic, strong) UILabel *nameLabel;
@property (nonatomic, strong) UILabel *contentLabel;
@end

@implementation DzwTestSectionFooter
...
-(void)sectionViewDidLoad{
    ...
    [[RACObserve(self, sectionViewModel) skip:1] subscribeNext:^(DzwTestTabModel *x) {
        @strongify(self);
        self.nameLabel.text = [NSString stringWithFormat:@"footer:%@",x.titleString];
        self.contentLabel.text = [NSString stringWithFormat:@"footer:%@",x.detailString];
    }];
